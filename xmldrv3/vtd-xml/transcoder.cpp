...
static inline void my_fputc(int ch, FILE *f){
	int i = fputc(ch, f);
	if (i==EOF){
		throw IOException("End of File reach in fputc");
	}
}
int com_ximpleware::UTF16BE_Coder_encode(UByte* output,int offset,int ch){
	if (ch<0x10000){ 
...
Long com_ximpleware::UTF16BE_Coder_decode(UByte *input,int offset){
	int val; 
	Long l;
...
int com_ximpleware::UTF16BE_Coder_getLen(int ch){
  if (ch <0x10000)
            return 2;
        else
            return 4;
...
void com_ximpleware::UTF16BE_Coder_encodeAndWrite(FILE* f, int ch){
	if (ch<0x10000){ 
...
int com_ximpleware::UTF16LE_Coder_encode(UByte* output,int offset,int ch){
	if (ch<0x10000){ 
...
Long com_ximpleware::UTF16LE_Coder_decode(UByte *input,int offset){
	int val; Long l;
...
int com_ximpleware::UTF16LE_Coder_getLen(int ch){
	    if (ch <0x10000)
            return 2;
        else
            return 4;
...
void com_ximpleware::UTF16LE_Coder_encodeAndWrite(FILE* f, int ch){
	if (ch<0x10000){ 
		my_fputc((ch & 0xff),f);
		my_fputc((ch & 0xff00) >> 8,f);
	} else {
...
int com_ximpleware::UTF8_Coder_encode(UByte* output,int offset,int ch){
	if (ch < 128){
		output[offset] = ch;
		return offset+1;
	}
...
Long com_ximpleware::UTF8_Coder_decode(UByte *input,int offset){
	Long l = 0;
	int c=0;
...
int com_ximpleware::UTF8_Coder_getLen(int ch){
	if (ch < 128)
		return 1;
	if (ch < 0x800)
		return 2;
...
void com_ximpleware::UTF8_Coder_encodeAndWrite(FILE* f, int ch){
	if (ch < 128){
		my_fputc(ch,f);
		return;
	}
...
int com_ximpleware::ASCII_Coder_encode(UByte* output,int offset,int ch){
	output[offset] = ch;
    return offset+1;
}

Long com_ximpleware::ASCII_Coder_decode(UByte *input,int offset){
    Long l = input[offset];
    return (((Long)(offset+1))<<32) | l ;
}

int com_ximpleware::ASCII_Coder_getLen(int ch){
	if (ch>127){
		throw TranscodeException("Invalid UCS char for ASCII format");
		return -1;
	}
	else
		return 1;
...
void com_ximpleware::ASCII_Coder_encodeAndWrite(FILE* f, int ch){
	if (ch>127)
		throw TranscodeException("Invalid UCS char for ASCII format");
	my_fputc(ch,f);
}

int com_ximpleware::ISO8859_1_Coder_encode(UByte* output,int offset,int ch){
	output[offset] = ch;
	return offset+1;
}
Long com_ximpleware::ISO8859_1_Coder_decode(UByte *input,int offset){
	Long l = input[offset] & 0xff;
	return (((Long)(offset+1))<<32) | l ;
}
int com_ximpleware::ISO8859_1_Coder_getLen(int ch){
	if (ch>255)
	{
		throw TranscodeException("Invalid UCS char for ISO-8859-1 format");
		return -1;
	}
	else
		return 1;
}
void com_ximpleware::ISO8859_1_Coder_encodeAndWrite(FILE* f, int ch){
	if (ch>255)
		throw TranscodeException("Invalid UCS char for ISO-8859-1 format");
	my_fputc(ch,f);
}

Long com_ximpleware::Transcoder_decode(UByte* input, int offset, encoding_t input_encoding){
 switch (input_encoding) {
        ...
        default:
            throw TranscodeException("Unsupported encoding");
			return -1;
        }
}

int  com_ximpleware::Transcoder_encode(UByte* output, int offset, int ch, encoding_t output_encoding){
...
void com_ximpleware::Transcoder_encodeAndWrite(FILE* f, int ch, encoding_t output_encoding){
        switch (output_encoding) {
        case FORMAT_ASCII:
...
int  com_ximpleware::Transcoder_getLen(int ch, encoding_t output_encoding){
	switch (output_encoding) {
		case FORMAT_ASCII:
...
int  com_ximpleware::Transcoder_getOutLength(UByte* input, int offset,int length,encoding_t input_encoding, encoding_t output_encoding){
	int len = 0;
	int k = offset;
	int c;
...
Long com_ximpleware::Transcoder_transcode(UByte* input, int offset, int length, encoding_t input_encoding, encoding_t output_encoding){
	...
	UByte* output =  new UByte[i];  //(UByte*) malloc(i);
	if (output==NULL){
		throw OutOfMemException("allocate byte buffer failed in Transcoder_transcode");
	}
...
void com_ximpleware::Transcoder_transcodeAndFill(UByte* input, UByte* output,int offset, int length, encoding_t input_encoding, encoding_t output_encoding){
	/*int len = 0;*/
	int k = offset;
	int c, i = 0;
	while (k < offset + length) {
...
int com_ximpleware::Transcoder_transcodeAndFill2(int initOutPosition, UByte* input, UByte* output, int offset, int length, 
...
void com_ximpleware::Transcoder_transcodeAndWrite(UByte* input, FILE* f,int offset, int length, encoding_t input_encoding, encoding_t output_encoding){
	/*int len = 0;*/
	int k = offset;
	int c;
	while (k < offset + length) {
